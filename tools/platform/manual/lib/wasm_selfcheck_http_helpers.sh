...
mfx_wasm_selfcheck_load_manifest_payload_http_code() {
    local output_file="$1"
    local base_url="$2"
    local token="$3"
...
mfx_wasm_selfcheck_reload_http_code() {
    local output_file="$1"
    local base_url="$2"
    local token="$3"
...
mfx_wasm_selfcheck_test_reset_runtime_http_code() {
    local output_file="$1"
    local base_url="$2"
    local token="$3"
...
mfx_wasm_selfcheck_import_dialog_probe_http_code() {
    ...
    if [[ -n "$initial_path" ]]; then
        ...
        initial_path_escaped="$(mfx_wasm_selfcheck_json_escape "$initial_path")"
...
mfx_wasm_selfcheck_test_resolve_text_config_http_code() {
    local output_file="$1"
    local base_url="$2"
    local token="$3"
    local payload_json="$4"
