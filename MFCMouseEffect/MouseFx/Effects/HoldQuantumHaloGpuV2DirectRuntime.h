...
class HoldQuantumHaloGpuV2DirectRuntime final {
public:
    ...
    bool Start(const RippleStyle& style, const POINT& startPt) {
        Stop();

        style_ = style;
        style_.windowSize = ClampI(style_.windowSize, 96, 640);
    ...
    void Update(uint32_t holdMs, const POINT& pt) {
...
private:
    ...
    static float ComputeProgress(uint64_t elapsedMs, uint32_t holdMs, uint32_t durationMs) {
        const float elapsedT = ClampF(static_cast<float>(elapsedMs) / static_cast<float>(durationMs), 0.0f, 1.0f);
        if (holdMs == 0u) return elapsedT;
        const float holdT = ClampF(static_cast<float>(holdMs) / static_cast<float>(durationMs), 0.0f, 1.0f);
