    ...
    bool Render(
        ...
        try {
            ...
            do {
                ...
                const float baseA = ClampF(static_cast<float>((stroke >> 24) & 0xFFu) / 255.0f, 0.06f, 1.0f);
                ...
                const float pr = ClampF(sr * 0.72f + 0.28f, 0.0f, 1.0f);
                const float pg = ClampF(sg * 0.18f + 0.10f, 0.0f, 1.0f);
                const float pb = ClampF(sb * 0.95f + 0.05f, 0.0f, 1.0f);

                const float cr = ClampF(sr * 0.35f + 0.00f, 0.0f, 1.0f);
                const float cg = ClampF(sg * 0.90f + 0.20f, 0.0f, 1.0f);
                const float cb = ClampF(sb * 0.95f + 0.30f, 0.0f, 1.0f);
                ...
                const float sweep = ClampF(progress, 0.0f, 1.0f) * 6.2831853f;
