...
public:
    ...
    bool Render(uint64_t elapsedMs, uint32_t holdMs, int sizePx, const RippleStyle& style) {
        ...
        const float pr = ClampF(sr * 0.70f + 0.28f, 0.0f, 1.0f);
        const float pg = ClampF(sg * 0.22f + 0.12f, 0.0f, 1.0f);
        const float pb = ClampF(sb * 0.94f + 0.06f, 0.0f, 1.0f);
        const float cr = ClampF(sr * 0.32f + 0.02f, 0.0f, 1.0f);
        const float cg = ClampF(sg * 0.88f + 0.22f, 0.0f, 1.0f);
