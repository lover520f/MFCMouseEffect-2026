...
- Phase 56+: ongoing.
  ...
  - macOS input-indicator mouse clicks/scrolls now share the Windows streak/label rules (`L x2/L x3`, `W+ x2`), and the Swift indicator view now animates over the same duration window instead of showing only a static frame, reducing click/scroll appearance drift between platforms.
  - input-indicator styling now supports a WASM route via new `indicator_*` input kinds, with a small indicator-tail payload (`size_px/duration_ms`) and `kTextIdEventLabel` for per-event labels, while native overlay fallback remains available.
  - WASM host routing is now surface-split (`effects` host + `indicator` host) under one shared runtime policy (`enabled/budget`), so effect plugins and indicator plugins no longer compete for one active plugin slot; `/api/wasm/load-manifest` now validates and reports against the target host chosen by `surface`.
