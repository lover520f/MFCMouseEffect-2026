...
- Backend-selection observability is now active too:
  ...
  - the app now also writes `%APPDATA%\\MFCMouseEffect\\websettings_runtime_auto.json` when WebSettings opens, so the Windows-native proof helper can auto-resolve `base_url/token` instead of requiring manual copy/paste on each machine
