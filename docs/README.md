...
- Issues: `docs/issues/dawn_gpu_final_present_startup_blackflash_guard_stage122.md` (mitigate startup black flash via cooldown rollback and longer startup guard)
- Issues: `docs/issues/dawn_gpu_startup_first_frame_prerender_stage123.md` (pre-render first frame before showing overlay windows to suppress startup flash)
- Issues: `docs/issues/layered_startup_transparent_first_present_stage124.md` (force first transparent layered present to prevent transient black default surface)
- Issues: `docs/issues/dawn_gpu_final_present_takeover_readiness_gate_stage125.md` (block non-layered final-present until host-chain takeover path is actually integrated)
- Issues: `docs/issues/dawn_gpu_final_present_takeover_gate_observability_stage126.md` (replace host hardcoded takeover gate with dedicated module + web/diag observability)
- Issues: `docs/issues/dawn_hold_neon3d_latency_priority_stage127.md` (prioritize hold_neon3d tracking latency under dawn layered fallback path)
- Issues: `docs/issues/dawn_trail_input_driven_sampling_stage128.md` (reduce per-frame fallback cursor polling in trail layer to lower hold-time CPU contention)
- Issues: `docs/issues/dawn_hold_neon3d_trail_pause_priority_stage129.md` (pause trail layer during active hold_neon3d on dawn layered fallback path to prioritize hold responsiveness)
- Issues: `docs/issues/dawn_gpu_present_nonlayered_black_guard_stage130.md` (hide non-layered GPU host surfaces without visible content + startup fail-fast rollback guard)
- Issues: `docs/issues/dawn_gpu_present_first_success_reveal_guard_stage131.md` (reveal non-layered GPU host windows only after first successful present)
- Issues: `docs/issues/dawn_gpu_takeover_build_flag_stage132.md` (enable takeover build flag while preserving local runtime off-switch safety)
- Issues: `docs/issues/dawn_gpu_takeover_explicit_on_gate_stage133.md` (require explicit local `.on` file before enabling takeover path)
- Issues: `docs/issues/dawn_layered_input_driven_immediate_frame_stage134.md` (input-driven immediate frame kicks to reduce layered fallback latency)
- Issues: `docs/issues/dawn_hold_trail_priority_submit_simplify_stage135.md` (remove redundant geometry noop submit and prioritize trail under hold mixed frames)
- Issues: `docs/issues/dawn_hold_trail_visibility_recovery_stage136.md` (restore trail visibility during hold latency-priority mode)
- Issues: `docs/issues/dawn_hold_trail_dispatch_interval_align_stage137.md` (align hold-priority trail dispatch interval to 8ms for lower follow latency)
- Issues: `docs/issues/dawn_hold_trail_source_gating_removal_stage138.md` (remove source-side hold trail secondary gating in AppController)
- Issues: `docs/issues/dawn_hold_trail_history_window_tighten_stage139.md` (tighten hold latency-priority trail history window to reduce preprocessing load)
- Issues: `docs/issues/dawn_hold_mixed_nontrail_dynamic_throttle_stage140.md` (dynamic hold mixed-frame non-trail throttling based on trail geometry load)
- Issues: `docs/issues/dawn_hold_mixed_nontrail_dynamic_throttle_tuning_stage141.md` (recalibrate heavy-trail threshold/interval to real hold geometry scale)
- Issues: `docs/issues/dawn_trail_pending_input_queue_stage142.md` (replace single latest cursor point with bounded pending queue for trail continuity)
- Issues: `docs/issues/dawn_hold_trail_start_seed_stage143.md` (seed trail points at hold-promotion boundary to reduce early hold-only frames)
- Issues: `docs/issues/dawn_hold_trail_start_seed_lastmove_stage144.md` (use latest move point as second hold-start trail seed before cursor polling fallback)
- Issues: `docs/issues/dawn_hold_drag_dispatch_adaptive_interval_stage145.md` (use 8ms drag dispatch interval under hold latency-priority mode; keep 20ms default)
- Issues: `docs/issues/dawn_hold_latency_prehold_activation_stage146.md` (activate hold latency-priority during pre-hold candidate window to reduce promotion-time lag jump)
- Issues: `docs/issues/dawn_hold_latency_prehold_drag_only_stage147.md` (limit pre-hold latency-priority to drag-like movement to preserve normal hold feel)
- Issues: `docs/issues/dawn_hold_trail_geometry_hard_cap_stage148.md` (hard-cap hold trail geometry to latest 24 points in Dawn consumer preprocessing)
- Issues: `docs/issues/dawn_hold_state_signal_for_consumer_stage149.md` (use trail latency-priority flag as hold signal so hold cap applies consistently for hold_neon3d)
- Issues: `docs/issues/dawn_hold_trail_vertex_cap_tighten_stage150.md` (tighten hold trail cap from 24 to 18 points to further lower hold geometry peak)
- Issues: `docs/issues/dawn_trail_latency_fallback_sampling_stage151.md` (fix latency-priority fallback cursor sampling cadence from 32ms to 8ms)
- Issues: `docs/issues/dawn_hold_latency_priority_link_hold_effect_stage152.md` (link latency-priority to hold effect so hold follow/signal cadence also enter low-latency mode)
- Issues: `docs/issues/dawn_hold_fast_follow_and_trail_cap_stage153.md` (add high-speed direct-follow path for hold_neon3d in latency mode and tighten latency trail cap to 18)
- Issues: `docs/issues/dawn_trail_vertex_cap_baseline_stage154.md` (add baseline non-hold trail cap in Dawn preprocessing: 28 points, hold remains 18)
- Issues: `docs/issues/dawn_trail_vertex_cap_tighten_stage155.md` (tighten non-hold trail cap in Dawn preprocessing from 28 to 24; hold stays 18)
- Issues: `docs/issues/dawn_trail_mixed_frame_cap_stage156.md` (for non-hold mixed frames, tighten Dawn trail cap further to 20 while pure trail keeps 24)
- Issues: `docs/issues/dawn_trail_cap_refine_stage157.md` (refine non-hold caps again: pure trail 22, mixed frame 18, hold 18)
- Issues: `docs/issues/dawn_trail_cap_observability_stage158.md` (expose active trail cap decision in diagnostics: cap value + mode)
- Issues: `docs/issues/dawn_gpu_takeover_default_on_gate_stage159.md` (remove strict `.on` requirement; takeover is default-on with `.off` kill switch)
- Issues: `docs/issues/dawn_gpu_takeover_default_on_rollback_stage160.md` (rollback default-on takeover after black-screen regression; require explicit `.on`)
- Issues: `docs/issues/dawn_hold_neon3d_follow_latency_tighten_stage161.md` (tighten hold continuous emit interval and layered neon3d follow snap to reduce long-press lag)
- Issues: `docs/issues/dawn_hold_input_immediate_frame_kick_stage162.md` (kick immediate frame after hold/ripple position+elapsed updates to reduce timer-wait latency)
