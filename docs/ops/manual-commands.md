...
- renderer lane matrix expectations:
  ...
  - newer runtime / proof / lane matrix summaries now also carry `scene_runtime_asset_node_rig_driver_brief`, `scene_runtime_asset_node_surface_driver_brief`, `scene_runtime_asset_node_pose_bus_brief`, `scene_runtime_asset_node_controller_table_brief`, `scene_runtime_asset_node_controller_registry_brief`, and `scene_runtime_asset_node_driver_bus_brief`, so you can tell whether the current lane has already crossed from surface bias into shared pose-bus / controller-table / controller-registry / driver-bus seams before any future real model-node controller binding lands
  - current seam states:
    - `preview_only`
    - `asset_stub_ready`
    - `pose_stub_ready`
