...
- renderer lane matrix expectations:
  ...
  - runtime / proof / lane matrix now also expose `scene_runtime_model_asset_source_brief` and `scene_runtime_model_asset_manifest_brief` before the older `model_scene_adapter` / `model_node_adapter` chain, so you can tell whether the current Windows real renderer is still preview-only, has a supported model asset source, or has already resolved the minimal `model + action library + appearance profile` manifest needed for later true 3D consumption
