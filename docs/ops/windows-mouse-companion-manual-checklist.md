...
10. Machine-check note:
  ...
  - `scene_runtime_asset_node_pose_resolver_brief` and `scene_runtime_asset_node_pose_registry_brief` should now also stay stable-format across lanes; they are the seams after pose-table lifting, so `body/head/appendage/overlay/grounding` should keep the same path ordering and then the same future pose-node ordering between runtime, proof, lane matrix, and WebUI even when pose resolution / registry weighting changes the actual numbers
