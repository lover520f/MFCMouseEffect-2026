...
10. Machine-check note:
  ...
  - `scene_runtime_asset_node_rig_driver_brief`, `scene_runtime_asset_node_surface_driver_brief`, `scene_runtime_asset_node_pose_bus_brief`, `scene_runtime_asset_node_controller_table_brief`, `scene_runtime_asset_node_controller_registry_brief`, and `scene_runtime_asset_node_driver_bus_brief` should now also stay stable-format across lanes; they are the seams after control-surface lifting, so `body/head/appendage/overlay/grounding` should keep the same driver/bus/controller-registry ordering between runtime, proof, lane matrix, and WebUI even when actual driver values drift
