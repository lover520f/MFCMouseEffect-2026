...
- Phase1 click-first backend semantics are active:
  ...
  - `fixed_bottom_left` move path no longer has to stay `idle`: move can enter the upright walking `follow` lane while the panel stays anchored, so mouse move can drive visible walk cadence without switching to free-follow positioning.
...
- Phase1 macOS visual host is active (model-first with placeholder fallback):
  ...
  - Follow parity stage1 (active): `follow` is currently defined as an upright cross-step walk, not crawl. The body/root should stay nearly fixed (no whole-body roll, no follow pulse scaling, and the real-model path no longer samples the loop `follow` clip at all), while the limbs do a two-beat opposite-limb gait: left leg forward pairs with right hand forward, right leg forward pairs with left hand forward. `fixed_bottom_left` shares the same walk-follow action lane while keeping panel position anchored. Runtime pulse scaling is now limited to reactive `hold/scroll` transitions only, so `follow <-> idle` threshold crossings no longer cause occasional enlarge/shrink flicker. Real-model root yaw is currently fixed front-facing for `idle/follow/click one-shot`; pointer yaw is reserved for explicit drag-only semantics so walking no longer makes the body look slanted.
