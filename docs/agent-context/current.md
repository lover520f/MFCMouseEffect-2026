...
- `tools/platform/manual/run-webui-dev-manual.sh` now owns Vite as a single tracked Node process (instead of `pnpm run dev` parent/child wrappers) and proactively stops stale workspace Vite instances before relaunch. This prevents stacked `5173/5174/5175` dev servers from surviving across debug restarts and keeps the opened `--debug` page aligned with the current probe/backend.
