...
- The shared WebUI button hover style no longer uses a global upward `translateY(-1px)` lift; only border/shadow feedback remains, because the old lift caused hover hit-target jitter and missed clicks in source-mode dev pages.
