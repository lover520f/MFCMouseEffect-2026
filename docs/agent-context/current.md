...
- `SceneRuntime` now also owns `modelAssetSourceProfile`, `modelAssetManifestProfile`, `modelAssetCatalogProfile`, and `modelAssetBindingTableProfile`, so Windows real renderer now walks `source -> manifest -> catalog -> binding table` before the old preview-only node chain: runtime/proof/matrix/WebUI now surface `scene_runtime_model_asset_source_*`, `scene_runtime_model_asset_manifest_*`, `scene_runtime_model_asset_catalog_*`, and `scene_runtime_model_asset_binding_table_*`, and the backend already lets those profiles bias glow/stroke/highlight/overlay/grounding readability instead of keeping them diagnostic-only.
