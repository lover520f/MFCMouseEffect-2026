...
- Windows pet real 3D asset seam now extends from `node_attach -> node_lift -> node_bind` to `node_resolve -> node_drive`; `modelNodeGraph` consumes `scene_runtime_model_asset_node_resolve_*`, while `modelNodeSlot` / `modelNodeRegistry` now consume `scene_runtime_model_asset_node_drive_*` before later asset-node binding layers.
