...
- Painter readability now follows the same pose lane lightly: `pose_bound` can raise shadow/pedestal alpha a bit, strengthen accessory stroke/fill, and make the pose badge read more confidently, so the extra pose influence still shows up when geometry deltas stay subtle.
- The three adapter modes now also have a shared readability bias helper, so `runtime_only / pose_unbound / pose_bound` no longer differ only by geometry: adornment and overlay intensity now step up on the same contract instead of each builder inventing its own thresholds.
