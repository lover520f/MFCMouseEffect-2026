...
- `assetNodeTargetResolverProfile` now feeds `assetNodeMatchCandidateProfile -> assetNodeMatchCatalogProfile`: candidate entries derive selector prefixes, primary/secondary node names, and candidate paths from asset resources, then the catalog turns them into canonical node keys/labels plus `matchConfidence` for downstream `assetNodeWorldSpaceProfile` / `assetNodeJointHintProfile`.
