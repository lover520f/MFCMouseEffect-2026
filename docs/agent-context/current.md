...
- POSIX dual-lane guardrail exists:
  ...
  - macOS scroll pulse core is now split by concern (`render plan` + `layers/animation` + orchestration entry), reducing scroll-effect render coupling while preserving effect contracts
