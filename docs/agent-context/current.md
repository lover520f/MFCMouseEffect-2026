...
- `assetNodeTargetResolverProfile` now feeds `assetNodeMatchCandidateProfile -> assetNodeMatchCatalogProfile -> assetNodeMatchEnumerationProfile -> assetNodeMatchPlanProfile -> assetNodeMatchResolveProfile -> assetNodeMatchQueryProfile`: candidate entries derive selector prefixes and candidate paths, catalog normalizes canonical keys/labels plus `matchConfidence`, enumeration emits parser-style locators, plan assembles probe/fallback intent, resolve exposes final node keys/labels plus `resolveConfidence`, and the new query layer now carries parser-facing locators/labels/confidence consumed by `assetNodeWorldSpaceProfile`, `assetNodeJointHintProfile`, `FrameBuilder`, `AdornmentBuilder`, and `ActionOverlayBuilder`.
