...
- Current `pose_bound` gain is intentionally narrow but visible: appendage pose consumption now scales by adapter mode, with `pose_unbound` reduced and `pose_bound` full.
