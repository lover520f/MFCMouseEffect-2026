...
- New additive lane `cursor_decoration` is active through the existing input-indicator overlay seam; current built-in decoration plugin ids are `ring` and `orb`, and the lane now appears under `Cursor Effects` as the sixth built-in channel while its `color_hex / size_px / alpha_percent` tuning now lives in `Effect Config`, not `Effect Plugins`; persistence remains `input_indicator.cursor_decoration`.
