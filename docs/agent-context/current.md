...
- `SceneRuntime` now also owns `modelAssetSourceProfile` and `modelAssetManifestProfile`, so Windows real renderer has a first-class 3D asset-entry seam before the old preview-only node chain: runtime/proof/matrix/WebUI now surface `scene_runtime_model_asset_source_*` and `scene_runtime_model_asset_manifest_*`, and the backend already lets those profiles bias glow/stroke/overlay readability instead of keeping them diagnostic-only.
